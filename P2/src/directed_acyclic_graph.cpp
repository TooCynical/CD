...
SequencePairDAG::SequencePairDAG(size_t n,
                                 ...
                                 _seq_pair(seq_pair)
{
    _total_weights = vector<uint64_t >(_n, 0);
    update();
}

bool SequencePairDAG::is_edge(uint64_t x, uint64_t y) {
    switch(_orientation) {
        case HORIZONTAL_LR:
...
vector<uint64_t> SequencePairDAG::total_weights_in_order() {
...
uint64_t SequencePairDAG::longest_path_length() {
    if (update() == FAIL) {
        cout << "SPDAG: Error: failed to update" << endl;
        exit(1);
    }
